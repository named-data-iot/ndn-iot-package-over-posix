...
/* Illuminance Sensor
 *
 * Certificate: /ndn-iot/illuminance_sensor9/KEY
 * Locator: /ndn-iot/living/illuminance_sensor9
 * Services: 
 *   Battery:
 *      uint32_t [1, 100]
 *   Illuminance: 
 ...
 * 
 * Signing Key:
 *   Battery: /ndn-iot/illuminance_sensor9/KEY/<keyID>/battery
 *   Illuminance: /ndn-iot/illuminance_sensor9/KEY/<keyID>/illuminance
 * 
 * Implementation: 
 *   Signing Key:
 ...
 *     Illuminance: publish only if Value - lastValue > 10 
 */
...
void service_data_publishing()
{
  ...
  /* simulate the illuminance */
  uint8_t dice;
  ndn_rng((uint8_t*)&dice, sizeof(dice));
  uint32_t illuminance = dice / 3;
...
void on_illuminance_command(uint8_t service, bool is_cmd, const name_component_t* identifiers, uint32_t identifiers_size,
                       const uint8_t* suffix, uint32_t suffix_len, const uint8_t* content, uint32_t content_len,
                       void* userdata)
{
  NDN_LOG_DEBUG("on illuminance command\n");

}


/* subscribe to the command namespace
 * /ndn-iot/Illuminance/CMD/<...>/OFF
 * /ndn-iot/Illuminance/CMD/<...>/ON
 *
 *
 */
void initialize()
{
  ndn_ac_register_encryption_key_request(NDN_SD_ILLUMINANCE);
  ...
  uint32_t keyid;
  for (int i = 0; i < 10; i++) {
    if (ac_state->self_services[i] == NDN_SD_ILLUMINANCE ||
...
int main(int argc, char *argv[])
{
  ...
  name_component_from_string(&id[1], "illuminance_sensor9", strlen("illuminance_sensor9"));
