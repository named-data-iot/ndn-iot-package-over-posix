...
void
prepare_cmd_data(uint8_t service, uint8_t* value, uint32_t value_size,
                      uint32_t* output_size)
{
  ndn_key_storage_t* storage = ndn_key_storage_get_instance();
  name_component_t* home_prefix = &storage->self_identity.components[0];
  ...
  ret = tlv_make_data(value, value_size, output_size, 6,
...
int main(int argc, char *argv[]){
    ...
    ps_subscribe_to(NDN_SD_TEMP, true, id, 2, 30000, on_publish, NULL);
    ...
    prepare_cmd_data(NDN_SD_TEMP, buffer, sizeof(buffer), &output_size);
    ndn_forwarder_receive(face, buffer, output_size);
    printf("one-time process\n");
    ndn_forwarder_process();
